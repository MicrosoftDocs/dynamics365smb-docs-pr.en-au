...
The **Bidirectional Synch of Sales Orders** option lets you synchronise sales orders from Sales to [!INCLUDE [prod_short](includes/prod_short.md)], and vice versa. For example, if a customer changes their mind about the product or quantity in [!INCLUDE[crm_md](includes/crm_md.md)], you can archive the sales document and create a new document in [!INCLUDE[prod_short](includes/prod_short.md)]. The same is true for changes in [!INCLUDE[prod_short](includes/prod_short.md)]. For example, when prices, tax amounts, or expected shipment dates change, the changes synchronise to [!INCLUDE[crm_md](includes/crm_md.md)]. Bidirectional synchronisation helps keep your sellers up to date with the latest changes and the status of sales orders.
