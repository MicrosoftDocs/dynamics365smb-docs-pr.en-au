...
Similarly, when you post a CR/Adj note, [!INCLUDE [prod_short](includes/prod_short.md)] calculates payment discounts based on the payment terms. It calculates the discount on CR/Adj notes in the same way as discounts on invoices. When you apply a CR/Adj note to an invoice, [!INCLUDE [prod_short](includes/prod_short.md)] reduces the discount amount for the invoice by the CR/Adj note's discount amount.  
