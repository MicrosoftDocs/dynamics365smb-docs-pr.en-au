...
* **Import only** specifies that you import information, but you'll manually create the corresponding CR/Adj note.
* **Auto create CR/Adj note** specifies that you import information and [!INCLUDE[prod_short](../includes/prod_short.md)] automatically creates the credit CR/Adj notes. This option requires that you turn on the **Auto Create Sales Order** toggle.
...
|Edit an order and decrease quantity|Imported order will use new quantity. Shopify refund with 0 amount will be imported that can't be converted to a CR/Adj note.| Connector will detect the change and mark the orders. |
|Edit an order and remove existing item |Removed item won't be imported. Shopify refund with 0 amount will be imported that can't be converted to a CR/Adj note.| Connector will detect the change and mark the orders. |
...
|Cancel unpaid order | Removed item won't be imported. Shopify refund with 0 amount will be imported that can not be converted into CR/Adj Note. |Connector will detect the change and mark the orders. |
...
You can create sales CR/Adj notes for refunds. The CR/Adj notes can have the following types of lines:
...
> [!Note]
> The return locations, including blank locations, defined in the **Shopify Shop Card** are used on the created CR/Adj note. The system ignores the original locations from orders or shipments.
