...
1. [!INCLUDE[prod_short](includes/prod_short.md)] checks the combination of the bill-to customer and the item and then calculates the applicable unit price and line discount percentage, using the following criteria:
    ...
    * Is the date within the starting and ending date of the price/discount agreement? For invoices and CR/Adj notes, this is the date in the **Posting Date** field on the document header. For all other documents, it's the date in the **Order Date** field on their headers.
...
2. In the **Currency Code** field, enter the code for a currency that the invoice discount terms on the line apply to. Leave the field blank to set up invoice discount terms in AUD.
