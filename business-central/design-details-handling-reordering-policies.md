...
For reordering policies that use a reorder point, you can define a time bucket. Time buckets help ensure that demands within the same time period are accumulated. The system then checks the effect on projected inventory and whether the reorder point has been passed.
